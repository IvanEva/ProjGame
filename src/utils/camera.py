...
class Camera:
    ...
    def apply(self, entity):
        """Возвращает сдвинутый прямоугольник спрайта для отрисовки."""
        return entity.rect.move(self.camera_rect.topleft)

    def update(self, target):
        """Позиционирует камеру так, чтобы центр цели был в центре экрана."""
        x = -target.rect.centerx + self.camera_rect.width // 2
        y = -target.rect.centery + self.camera_rect.height // 2

        # Ограничиваем перемещение камеры границами карты
        x = min(0, x)                      # не дальше левого края
        y = min(0, y)                      # не дальше верхнего края
